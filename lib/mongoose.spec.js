...
describe('Mongoose Adapter', () => {
    ...
    const fakeName = 'test';
    const fakeSchemaName = 'testschema';
    const fakeSchema = 'Schema {}';
    const fakeRealSchema = new mongoose.Schema({});
    const fakeConfig = { models: {
        [fakeName]: fakeSchema,
        [fakeSchemaName]: fakeRealSchema
    }};
    ...
    it('should register models correctly', () => {
        ...
        expect(adapter.conn.model).toHaveBeenCalledTimes(2)
        expect(adapter.conn.model.mock.calls).toHaveProperty("0", [fakeName, fakeSchemaReturn]);
        expect(adapter.conn.model.mock.calls).toHaveProperty("1", [fakeSchemaName, fakeRealSchema]);
    });
});